...
export const PAGES_SEED: PageRecord[] = [
  {
    ...
    body_en: '<p>This site is a personal portfolio built to collect as little data as possible.</p><h2>What is stored in your browser</h2><p>Three strictly-necessary functional items are stored — no consent is required under the ePrivacy Directive for these:</p><ul><li><code>miciodev-theme</code> in <code>localStorage</code> — your light/dark theme preference, applied before first paint to avoid a colour flash. Kept until you clear browser storage; never shared.</li><li><code>miciodev-cookie-ack</code> in <code>localStorage</code> — records that you have seen this privacy notice. Kept until you clear browser storage; never shared.</li><li><code>miciodev-locale</code> HTTP cookie — your language preference (en / it / es), set on every page visit. Expires after 1 year; never shared.</li></ul><h2>External image requests</h2><p>The Videos section loads thumbnail images from <code>i.ytimg.com</code> (YouTube / Google). These are plain image requests sent with <code>referrerpolicy="no-referrer"</code>; no YouTube player or tracking script is embedded. Google\'s privacy policy governs those requests.</p><h2>The contact form</h2><p>If you submit the contact form, the name, email address, subject and message you enter are sent to this site\'s own self-hosted backend. No third party receives this data. It is kept only as long as needed to reply, then deleted.</p><h2>What this site does not do</h2><ul><li>No analytics, tracking pixels, or advertising.</li><li>No third-party scripts, CDNs, or remote font services — all assets are self-hosted.</li><li>No profiling; no data sold or shared with third parties.</li></ul><h2>Your rights</h2><p>Under the GDPR you have the right to access, rectify, erase, restrict, or obtain a portable copy of your personal data, and to object to its processing. You also have the right to lodge a complaint with a supervisory authority (e.g. the Italian Garante per la protezione dei dati personali or the Spanish AEPD). To exercise any of these rights, use the contact form on this site.</p><h2>Data controller</h2><p>The controller is the individual operating this site under the MicioDev name. Full legal name, address and VAT / tax identification number are available on request via the contact form — see also the <a href="/imprint">legal notice</a>.</p><p><em>This page is informational and is not legal advice.</em></p>',
    body_it: '<p>Questo sito è un portfolio personale, pensato per raccogliere il minimo di dati possibile.</p><h2>Cosa viene salvato nel tuo browser</h2><p>Tre elementi strettamente necessari e funzionali — non è richiesto il consenso ai sensi della Direttiva ePrivacy per questi:</p><ul><li><code>miciodev-theme</code> in <code>localStorage</code> — la tua preferenza di tema chiaro/scuro, applicata prima del primo render per evitare un lampeggio di colori. Conservato finché non svuoti l\'archivio del browser; mai condiviso.</li><li><code>miciodev-cookie-ack</code> in <code>localStorage</code> — registra che hai visualizzato questa informativa. Conservato finché non svuoti l\'archivio del browser; mai condiviso.</li><li>Cookie HTTP <code>miciodev-locale</code> — la tua preferenza linguistica (en / it / es), impostata a ogni visita. Scade dopo 1 anno; mai condiviso.</li></ul><h2>Immagini da servizi esterni</h2><p>La sezione Video carica miniature da <code>i.ytimg.com</code> (YouTube / Google). Si tratta di semplici richieste di immagini inviate con <code>referrerpolicy="no-referrer"</code>; non viene incorporato alcun player YouTube né script di tracciamento. Le richieste sono soggette alla privacy policy di Google.</p><h2>Il modulo di contatto</h2><p>Se invii il modulo di contatto, nome, indirizzo email, oggetto e messaggio vengono inviati al backend self-hosted di questo sito. Nessuna terza parte riceve questi dati. Vengono conservati solo per il tempo necessario a rispondere, poi eliminati.</p><h2>Cosa questo sito non fa</h2><ul><li>Nessuna analitica, pixel di tracciamento o pubblicità.</li><li>Nessuno script di terze parti, CDN o servizi di font remoti — tutti gli asset sono auto-ospitati.</li><li>Nessuna profilazione; nessun dato venduto o condiviso con terze parti.</li></ul><h2>I tuoi diritti</h2><p>Ai sensi del GDPR hai il diritto di accedere, rettificare, cancellare, limitare o ottenere una copia portabile dei tuoi dati personali, e di opporti al loro trattamento. Hai inoltre il diritto di proporre reclamo a un\'autorità di controllo (ad esempio il Garante per la protezione dei dati personali in Italia o l\'AEPD in Spagna). Per esercitare uno di questi diritti usa il modulo di contatto del sito.</p><h2>Titolare del trattamento</h2><p>Il titolare è il professionista che gestisce questo sito con il nome MicioDev. Ragione sociale completa, indirizzo e partita IVA / codice fiscale sono disponibili su richiesta tramite il modulo di contatto — vedi anche le <a href="/imprint">note legali</a>.</p><p><em>Questa pagina è informativa e non costituisce consulenza legale.</em></p>',
    body_es: '<p>Este sitio es un portfolio personal, creado para recoger la menor cantidad de datos posible.</p><h2>Qué se guarda en tu navegador</h2><p>Tres elementos estrictamente necesarios y funcionales — no se requiere consentimiento bajo la Directiva ePrivacy para estos:</p><ul><li><code>miciodev-theme</code> en <code>localStorage</code> — tu preferencia de tema claro/oscuro, aplicada antes del primer renderizado para evitar un destello de color. Se conserva hasta que borres el almacenamiento del navegador; nunca se comparte.</li><li><code>miciodev-cookie-ack</code> en <code>localStorage</code> — registra que has visto este aviso de privacidad. Se conserva hasta que borres el almacenamiento del navegador; nunca se comparte.</li><li>Cookie HTTP <code>miciodev-locale</code> — tu preferencia de idioma (en / it / es), establecida en cada visita. Caduca al año; nunca se comparte.</li></ul><h2>Imágenes de servicios externos</h2><p>La sección Vídeos carga miniaturas desde <code>i.ytimg.com</code> (YouTube / Google). Son solicitudes de imagen enviadas con <code>referrerpolicy="no-referrer"</code>; no se incrusta ningún reproductor de YouTube ni script de seguimiento. La política de privacidad de Google rige esas solicitudes.</p><h2>El formulario de contacto</h2><p>Si envías el formulario de contacto, el nombre, la dirección de correo, el asunto y el mensaje se envían al backend autoalojado de este sitio. Ningún tercero recibe estos datos. Se conservan solo el tiempo necesario para responder y luego se eliminan.</p><h2>Qué no hace este sitio</h2><ul><li>Sin analítica, píxeles de seguimiento ni publicidad.</li><li>Sin scripts de terceros, CDNs ni servicios de fuentes remotas — todos los recursos son autoalojados.</li><li>Sin perfilado; ningún dato vendido o compartido con terceros.</li></ul><h2>Tus derechos</h2><p>Bajo el RGPD tienes derecho a acceder, rectificar, suprimir, limitar u obtener una copia portátil de tus datos personales, y a oponerte a su tratamiento. También tienes derecho a presentar una reclamación ante una autoridad de control (p. ej. la AEPD en España o el Garante en Italia). Para ejercer cualquiera de estos derechos, utiliza el formulario de contacto del sitio.</p><h2>Responsable del tratamiento</h2><p>El responsable es el profesional que gestiona este sitio bajo el nombre MicioDev. Nombre legal completo, dirección y NIF / número de identificación fiscal disponibles bajo petición a través del formulario de contacto — véase también el <a href="/imprint">aviso legal</a>.</p><p><em>Esta página es informativa y no constituye asesoramiento legal.</em></p>',
  },
  {
