...
// "… · v1.1.0" in all three locales. The seed (1778600100_created_translations)
// was patched in-place when the site moved to v1.1.0, but instances that had
...
// on stage despite the bundled JSON shipping "v1.1.0". Companion to
...
migrate(
  (app) => {
    try {
      ...
      rec.set('en', 'Built with Astro + PocketBase · v1.1.0');
      rec.set('it', 'Realizzato con Astro + PocketBase · v1.1.0');
      rec.set('es', 'Hecho con Astro + PocketBase · v1.1.0');
      app.save(rec);
    } catch (_) {
      // No row yet (e.g. fresh DB seeded after the bump) — bundled JSON
      // already carries v1.1.0, so nothing to do.
    }
  },
  (app) => {
    try {
