...
// Custom admin page: drag-and-drop ordering for the `videos` collection (the
// YouTube videos shown in the "Videos" box on the home page).
//
// PocketBase's built-in dashboard can add/edit/delete the rows but can't
// reorder them by dragging. This hook serves a tiny self-contained page at:
//
//     {PB_URL}/videos-order        e.g. https://pb.micio86dev.it/videos-order
//
// It asks for a superuser email + password (the `videos` collection's write
// rule is superusers-only), lists the rows (with a thumbnail), lets you drag
// them into order, and on "Save order" PATCHes the `order` field of each
// changed row. No external scripts, no build step — just one HTML document.
// The page itself is harmless to serve publicly; the security boundary is
// PocketBase's API (the PATCH calls need a valid superuser token).
//
// Linked from the Astro frontend's docs only — not from the public site.

routerAdd('GET', '/videos-order', (e) => {
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  ul#list li .thumb {
    flex: none; width: 72px; height: 40px; border-radius: 4px; object-fit: cover;
    background: #0e0f10; border: 1px solid #2a2b2e;
  }
  ul#list li .meta { flex: 1; min-width: 0; }
  ul#list li .label { font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
<body>
<div class="wrap">
  <h1>Videos — reorder</h1>
  <p class="sub">Drag the rows into the order you want them in the "Videos" box, then save. Adding / editing / deleting videos (URL &amp; title) is done in the <a href="/_/" target="_blank" rel="noopener">PocketBase dashboard</a>.</p>
  ...
  <div id="app" hidden>
    ...
    <p class="muted">Order is stored in the <code>videos.order</code> column. The site reads it fresh on every request — no redeploy needed.</p>
  </div>
</div>

<script>
(function () {
  var TOKEN_KEY = 'pb_videos_order_token';
  ...
  // Pull the 11-char video id out of a YouTube watch / youtu.be / shorts URL.
  function ytId(url) {
  ...
  function render() {
    ...
    records.forEach(function (r) {
      var id = ytId(r.url);
      var thumb = id ? 'https://i.ytimg.com/vi/' + id + '/default.jpg' : '';
      var li = document.createElement('li');
      li.draggable = true;
      li.dataset.id = r.id;
      li.innerHTML =
        '<span class="grip" aria-hidden="true">⠿</span>' +
        '<img class="thumb" alt="" src="' + esc(thumb) + '">' +
        '<span class="meta"><div class="label">' + esc(r.title || '(no title)') + '</div>' +
  ...
  function load() {
    setStatus('Loading…');
    return fetch('/api/collections/videos/records?perPage=500&sort=order,created', { headers: authHeaders() })
      .then(function (res) {
      ...
      .then(function (data) {
        records = data.items || [];
        render();
        setStatus(records.length ? '' : 'No videos yet — add some in the PocketBase dashboard.');
      })
  ...
  function save() {
    ...
    changed.forEach(function (c) {
      chain = chain.then(function () {
        return fetch('/api/collections/videos/records/' + encodeURIComponent(c.id), {
          method: 'PATCH',
